...
export default function EditEmpreendimento() {
    ...
    useEffect(() => {
        const loadEmpreendimento = async () => {
            const response = await api.get(`/admin/enterprises/${_id}`);
            const empreendimento = response.data;

            setname(empreendimento.name);
            setdraft(empreendimento.draft);
            setdescription(empreendimento.description);
            setlogo(empreendimento.logo);
            setcategory(empreendimento.category);
            setphotos(empreendimento.photos ? empreendimento.photos : []);
            setSelectedsDifferentials(empreendimento.differentials ? empreendimento.differentials : []);
            sethotsiteUrl(empreendimento.hotsiteUrl);
            setbookUrl(empreendimento.bookUrl);
            setlatitude(empreendimento.loc[0]);
            setlongitude(empreendimento.loc[1]);
            setaddress(empreendimento.address);
            setstate(empreendimento.state);
            setcountry(empreendimento.country);
            setcity(empreendimento.city);
            setEnterprisePublic(empreendimento.public);
            setstatus(empreendimento.status)
            settourVirtualUrl(empreendimento.tourVirtualUrl);


            if (empreendimento.start) {
                setstart(moment(empreendimento.start).locale('pt-br').format('YYYY-MM-DD'));
            }

            if (empreendimento.end) {
                setend(moment(empreendimento.end).locale('pt-br').format('YYYY-MM-DD'));
            }

            if (empreendimento.differentials && empreendimento.differentials.length > 0) {
                const d = []
                empreendimento.differentials.forEach(diferencial => {
                    d.push(diferencial._id)
                })
                setdifferentials(d)
            }
    ...
    const editar = async () => {
        ...
        try {
            ...
            await api.put('/admin/enterprises', {
                _id,
                name,
                draft,
                description,
            ...
            setIsLoading(false);
            setMsg({ message: 'Empreendimento alterado com sucesso.', type: 'success' });
    ...
    const removeEmpreendimento = async () => {
        var response = window.confirm("Tem certeza que quer remover esse empreendimento da plataforma?");
        if (response === true) {
            await api.delete(`/admin/enterprises/${_id}`);
            history.goBack();
        }
    }
    ...
    return (
        <Page name="assessment-types">
            <h1>
                ...
                <BtnTitle label="Remover" iconName="trash" iconColor="#e63946" handleOnClick={() => removeEmpreendimento()} />
            </h1>
            ...
            <div className="areaInfo">
                ...
                <div className="informacoes">
                    ...
                    <div className="container">
                        ...
                        <Address label="Endreço" placeholder={address} handleValue={(place) => setInfoAddress(place)} />
                        <div className="areaAddress">
                    ...
                    <div className="container">
                        ...
                        <Fieldset label="Tour Virtual URL" type="text" placeholder="Informe a url do tourvirtual" handleValue={tourVirtualUrl} handleOnChange={(e) => settourVirtualUrl(e.target.value)} />
                    </div>
                    ...
                    <div className="areaBtnPlan">
                        <Button label="Editar" isLoading={isLoading} handleOnClick={editar} handleType="submit" />
                    </div>
